...
| Google photos | Some Google gym photos were not showing and displayed a broken image instead. | The app was saving temporary Google photo links in the database, and these links can expire or stop working. | Changed the app so it saves the Google Place ID and loads a fresh Google photo when the page opens. | I checked the deployed site and confirmed the gym cards could load Google photos again. | Fixed |
...
| Forms | The rating dropdown text was hard to read in some browsers and looked like it dissapeared as it as white on white. | Browsers can display dropdown menus differently, especially between light mode and dark mode. | Updated the dropdown styling so the options have clear text and background contrast. | I tested the dropdown on the deployed site and checked that the rating numbers were readable. | Fixed |
